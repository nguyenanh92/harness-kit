...
Trong dự án hiện tại của chúng ta (`harness-kit`), công cụ đánh giá cấu trúc Harness (`validate_harness.py`) kiểm tra chất lượng dựa trên **5 hệ thống con (Subsystems)**. Chúng ta có thể ánh xạ 9 thành phần cốt lõi của Harness ở trên vào 5 subsystem này như sau:
...
| **4. Scope (Phạm vi)** | • Permissions & Safety<br>• Sub-agent Isolation | Đặt ra biên giới hoạt động cho Agent. Giới hạn các file được phép sửa đổi (`feature_list.json`), chặn thoát khỏi thư mục dự án và phân quyền Shell. |
